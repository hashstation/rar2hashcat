...
static const char itoa16[16] = "0123456789""abcdef";
const char itoa16u[16] = "0123456789""ABCDEF";
...
static unsigned int strlen16(const UTF16 *str)
{
	unsigned int len = 0;
	while (*str++ != 0)
		len++;
...
static char *strnzcpy(char *dst, const char *src, int size)
{
	char *dptr;
		...
		case e_b64_mime:	/* mime */
		{
			...
			if (strchr(fromWrk, '.')) {
				...
				if (from_len<sizeof(fromTmp)-3)
					...
					alloced = 1;
					fromWrk = (char*)calloc(1, from_len+3);
				}
